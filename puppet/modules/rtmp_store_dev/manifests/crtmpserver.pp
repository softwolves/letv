class rtmp_store_dev::crtmpserver{
  ...
  file{"/home/update/shell/crtmpserver_update.sh":
    ...
    content => template("rtmp_store_dev/crtmpserver_update.sh"),
  }

  file{"/home/update/puppetmd5file/md5-crtmpserver":
    ensure => file,
    mode => 644,owner => root,group => root,
    content => template("rtmp_store_dev/md5-crtmpserver"),
  }
  ...
  file { "/usr/local/etc/crtmpserver.cdn.lua":
    ...
    mode => 644,
    content => template("rtmp_store_dev/crtmpserver.cdn.lua"),
    }

  file { "/usr/local/etc/antileech.lua":
    ensure => file,
    mode => 644,
    content => template("rtmp_store_dev/antileech.lua"),
    }

  file { "/usr/local/etc/relay.lua":
    ensure => file,
    mode => 644,
    content => template("rtmp_store_dev/relay.lua"),
    }

  file { "/usr/local/etc/r2h.cdn.conf":
    ensure => file,
    mode => 644,
    content => template("rtmp_store_dev/r2h.cdn.conf"),
    }

  file { "/usr/local/etc/r2h.cdn.9100.conf":
    ensure => file,
    mode => 644,
    content => template("rtmp_store_dev/r2h.cdn.9100.conf"),
    }
