...
function unixtime(c_time)
  if c_time == nil then
    return nil
  end

  local _, _, y, m, d, h, mi, s = string.find(c_time, "(%d+)-(%d+)-(%d+)%s+(%d%d):(%d%d):(%d%d)")
  local unixtime = os.time{year=y, month=m, day=d, hour=h, min=m, sec=s}
  return unixtime
end

function TimePeriods(stime, etime)
  local cur_time = ngx.localtime()
 
  local start_time = unixtime(stime)
  local end_time = unixtime(etime)
  local loc_time = unixtime(cur_time)
...
function Top12time(top12_config)
  for i=1, #top12_config do
     if TimePeriods(top12_config[i].stime, top12_config[i].etime) then
       return true
     end
  end
  return false
...
function cde_check(cde_version)
  ...
  if  cde_version == 1024 or cde_version == 1032 or cde_version == 1043 then
     return true
  end
...
function IsCCTV5(stream_id)
  if stream_id == nil then return end
  local k, v = string.match(stream_id, "cctv5_(.*)")
  if k ~= nil then
    return true
  end
end
...
function IsTop12Streamid(stream_id, top12_config)
  if top12_config == nil then return end
  for i=1, #top12_config do 
    local k, v = string.match(stream_id, top12_config[i].stream_id)
    if k~= nil then
      return top12_config[i]
    end
  end
  return false
end
...
function  Strategy.Tvapk()
  ...
  if user.cde == 1025 or user.cde == 1036  or user.cde == 1046 then
    return 
  end
  return  flowrate_control(user, 0, 25)
end
...
function Strategy.Top12()
  local top12_config ={
    {stime =  "2016-09-06  19:35:00", etime = "2016-09-06  21:45:00"},
    {stime =  "2016-10-06  19:35:00", etime = "2016-10-06  21:45:00"},
    {stime =  "2016-10-11  21:00:00", etime = "2016-10-11  23:10:00"},
  }
  local sp = user.splatid
  if sp ~= 1011 then
    return
  ...
  if not IsCCTV5(user.stream_id) then
    return
  end

  if not Top12time(top12_config)  then
    return
  end
