class cto2_all_dev::config {
  ...
  file { "/cto2/ctocmd":
    ...
    source => "puppet://$fileserver/cto2_all_dev/ctocmd",
  }

#  file { "/cto2/ctod":
#    ensure => file,
#    mode => 755,owner => root,group => root,
#    source => "puppet://$fileserver/cto2_all_dev/ctod",
#  }

  file { "/cto2/ctoguard":
    ensure => file,
    mode => 755,owner => root,group => root,
    source => "puppet://$fileserver/cto2_all_dev/ctoguard",    
  }

  file { "/cto2/ctostart":
    ensure => file,
    mode => 755,owner => root,group => root,
    source => "puppet://$fileserver/cto2_all_dev/ctostart",    
  }

  file { "/cto2/default.conf":
    ensure => file,
    mode => 644,owner => root,group => root,
    content => template("cto2_all_dev/default.conf"),
  }

  file { "/cto2/ctoclear":
    ensure => file,
    mode => 755,owner => root,group => root,
    content => template("cto2_all_dev/ctoclear"),
  }

  file { "/cto2/log.conf":
    ensure => file,
    mode => 755,owner => root,group => root,
    content => template("cto2_all_dev/log.conf"),
  }

  file { "/etc/init.d/cto2.3":
    ensure => file,
    mode => 755,owner => root,group => root,
    content => template("cto2_all_dev/cto2.3"),
  }

  file { "/cto2/lib/libboost_program_options.so.1.58.0":
    ensure => file,
    mode => 755,owner => root,group => root,
    source => "puppet://$fileserver/cto2_all_dev/libboost_program_options.so.1.57.0",    
  }

#  file { "/cto2/lib/libstdc++.so.6":
#    ensure => file,
#    mode => 755,owner => root,group => root,
#    source => "puppet://$fileserver/cto2_all_dev/libstdc++.so.6",    
#  }
