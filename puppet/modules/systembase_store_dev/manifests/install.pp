class systembase_store_dev::install{
  package{"aria2":
    ensure   => present,
    provider => rpm,
...
#  exec {"install pyip gevent":
#    path      => "/usr/bin:/usr/sbin:/bin:/usr/local/bin:/bin/:/sbin",
#    command => "/usr/bin/pip install -i http://mirrors.aliyun.com/pypi/simple pyip;/usr/bin/pip install -i http://mirrors.aliyun.com/pypi/simple gevent",
#    require => Exec['/usr/bin/pip'],
#    creates => "/usr/lib/python2.6/site-packages/ip.py"
#  }

  exec {"/usr/bin/monit":
#    command => "/bin/rpm -ivh --replacefiles --replacepkgs http://${rpmsource}/puppet/package/monit-5.5.1-2.x86_64.rpm",
