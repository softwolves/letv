class systembase_store_dev::ffmux{
  file{"/lib/libffmux.so.0":
    ensure => file,
    mode   => 755,owner => root,group => root,
    source => "puppet://$fileserver/systembase_store_dev/libffmux.so.0",
  }
  ...
  file{"/lib/libudtp.so.0":
    ...
    mode   => 755,owner => root,group => root,
    source => "puppet://$fileserver/systembase_store_dev/libudtp.so.0",
  }

  file{"/lib/libjson.so.0":
    ensure => file,
    mode   => 755,owner => root,group => root,
    source => "puppet://$fileserver/systembase_store_dev/libjson.so.0",
  }
  ...
  file{"/lib/libevent-2.0.so.5":
    ...
    mode   => 755,owner => root,group => root,
    source => "puppet://$fileserver/systembase_store_dev/libevent-2.0.so.5",
  }

  file{"/lib/libxml2.so.2":
    ensure => file,
    mode   => 755,owner => root,group => root,
    source => "puppet://$fileserver/systembase_store_dev/libxml2.so.2",
  }
