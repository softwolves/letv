class systembase_live_dev::yumrepo{
  file{"/etc/yum.repos.d/letv-pkgs.repo":
    ensure => file,
    mode => 644,
    content => template("systembase_live_dev/letv-pkgs.repo"),
  }

  file{"/etc/yum.repos.d/CentOS-Base.repo":
    ensure => file,
    mode => 644,owner => root,group => root,
    content => template("systembase_live_dev/CentOS-Base.repo"),
  }

  file{"/etc/yum.repos.d/CentOS-Debuginfo.repo":
    ensure => file,
    mode => 644,owner => root,group => root,
    content => template("systembase_live_dev/CentOS-Debuginfo.repo"),
  }

  file{"/etc/yum.repos.d/CentOS-Media.repo":
    ensure => file,
    mode => 644,owner => root,group => root,
    content => template("systembase_live_dev/CentOS-Media.repo"),
  }

  file{"/etc/yum.repos.d/CentOS-Vault.repo":
    ensure => file,
    mode => 644,owner => root,group => root,
    content => template("systembase_live_dev/CentOS-Vault.repo"),
  }
