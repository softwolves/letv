class systembase_newcdn::ffmux{
  file{"/lib/libffmux.so.0":
    ensure => file,
    mode   => 755,owner => root,group => root,
    source => "puppet://$fileserver/systembase_newcdn/libffmux.so.0",
  }
...
#    command     => "killall -9 nginx;killall -9 nginx;ldconfig;/usr/local/sbin/nginx -c /usr/local/etc/nginx.conf",
  ...
  file{"/lib/libudtp.so.0":
    ...
    mode   => 755,owner => root,group => root,
    source => "puppet://$fileserver/systembase_newcdn/libudtp.so.0",
  }
  ...
  file{"/lib/libevent-2.0.so.5":
    ...
    mode   => 755,owner => root,group => root,
    source => "puppet://$fileserver/systembase_newcdn/libevent-2.0.so.5",
  }

  file{"/lib/libxml2.so.2":
    ensure => file,
    mode   => 755,owner => root,group => root,
    source => "puppet://$fileserver/systembase_newcdn/libxml2.so.2",
  }

  file{"/lib/libjson.so.0":
    ensure => file,
    mode   => 755,owner => root,group => root,
    source => "puppet://$fileserver/systembase_newcdn/libjson.so.0",
  }
