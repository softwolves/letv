class nginx_cloud_dev::install {
  ...
  file { "/home/update/shell/nginx_update.sh":
    ...
    content => template("nginx_cloud_dev/nginx_update.sh"),
  }

  file { "/home/update/puppetmd5file/md5-nginx":
    ensure => file,
    mode => 644,
    content => template("nginx_cloud_dev/md5-nginx"),
  }
  ...
  exec { "update-nginx":
    ...
    refreshonly => true,
    before => Class["nginx_cloud_dev::config"]
  }
}
