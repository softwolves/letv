...
local delay = 5
local handler
handler = function (premature, args, host, connection, cache_key, remote_port, server_port)
    if premature then
        return
    end
    ...
    if tostring(res_tab["resultCode"]) == "1" then
        ...
        trans_arg_tab["client_port"] = remote_port
        trans_arg_tab["server_port"] = server_port
...
if trans_args then
    ...
    local ok, err = ngx.timer.at(0, handler, table.concat(arg_tab, "&"), ngx.var.domain, ngx.var.connection, cache_key, ngx.var.remote_port, ngx.var.server_port)
    if not ok then
