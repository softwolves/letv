class systembase_front::ffmux{
  file{"/lib/libffmux.so.0":
    ensure => file,
    mode   => 755,owner => root,group => root,
    source => "puppet://$fileserver/systembase_front/libffmux.so.0",
  }
  ...
  file{"/lib/libudtp.so.0":
    ...
    mode   => 755,owner => root,group => root,
    source => "puppet://$fileserver/systembase_front/libudtp.so.0",
  }

  file{"/lib/libjson.so.0":
    ensure => file,
    mode   => 755,owner => root,group => root,
    source => "puppet://$fileserver/systembase_front/libjson.so.0",
  }
  ...
  file{"/lib/libevent-2.0.so.5":
    ...
    mode   => 755,owner => root,group => root,
    source => "puppet://$fileserver/systembase_front/libevent-2.0.so.5",
  }

  file{"/lib/libxml2.so.2":
    ensure => file,
    mode   => 755,owner => root,group => root,
    source => "puppet://$fileserver/systembase_front/libxml2.so.2",
  }
