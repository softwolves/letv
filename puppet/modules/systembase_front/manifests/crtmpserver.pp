class systembase_front::crtmpserver{
  file{"/usr/local/etc/updatecrtmpserver.sh":
    ensure => file,
    mode => 755,owner => root,group => root,
    content => template("systembase_front/updatecrtmpserver.sh"),
  }

  file{"/usr/local/etc/md5-crtmpserver":
    ensure => file,
    mode => 644,owner => root,group => root,
    content => template("systembase_front/md5-crtmpserver"),
  }
  ...
  file { "/usr/local/etc/crtmpserver.lua":
    ...
    mode => 644,
    content => template("systembase_front/crtmpserver.lua"),
    }

  file { "/usr/local/etc/antileech.lua":
    ensure => file,
    mode => 644,
    content => template("systembase_front/antileech.lua"),
    }

  file { "/usr/local/etc/relay.lua":
    ensure => file,
    mode => 644,
    content => template("systembase_front/relay.lua"),
    }
