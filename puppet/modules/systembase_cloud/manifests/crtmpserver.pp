class systembase_cloud::crtmpserver{
  file{"/usr/local/etc/updatecrtmpserver.sh":
    ensure => file,
    mode => 755,owner => root,group => root,
    content => template("systembase_cloud/updatecrtmpserver.sh"),
  }

  file{"/usr/local/etc/md5-crtmpserver":
    ensure => file,
    mode => 644,owner => root,group => root,
    content => template("systembase_cloud/md5-crtmpserver"),
  }
  ...
  file { "/usr/local/etc/crtmpserver.lua":
    ...
    mode => 644,
    content => template("systembase_cloud/crtmpserver.lua"),
    }

  file { "/usr/local/etc/antileech.lua":
    ensure => file,
    mode => 644,
    content => template("systembase_cloud/antileech.lua"),
    }

  file { "/usr/local/etc/relay.lua":
    ensure => file,
    mode => 644,
    content => template("systembase_cloud/relay.lua"),
    }
