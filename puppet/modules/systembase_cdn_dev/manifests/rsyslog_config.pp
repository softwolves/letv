class systembase_cdn_dev::rsyslog_config {

#  file { "/etc/rsyslog.conf":
#    ensure => file,
#    mode => 644,
#    content => template("systembase_cdn_dev/rsyslog.conf"),
#    notify  => Class['systembase_cdn_dev::rsyslog_service'],
#  }

#  file { "/etc/logrotate.d/syslog":
#    ensure => file,
#    mode => 644,
#    content => template("systembase_cdn_dev/syslog"),
#  }

  file { "/etc/rsyslog.d/lers.conf":
    ensure => file,
    mode => 644,
    content => template("systembase_cdn_dev/lers.conf"),
  ...
  service { "rsyslog":
    ...
    hasstatus => true,
    require => Package["rsyslog"],
    subscribe => File["/etc/rsyslog.d/lers.conf"],
  }
  ...
  file{"/letv/soft/tools/rsyslog_update.sh":
    ...
    content => template("systembase_cdn_dev/rsyslog_update.sh"),
  }
