...
#
#fi
num_cop=`ps aux | grep -E '/usr/local/sbin/ats//bin/traffic_cop' | grep -Ev grep  | wc -l`
if [ $num_cop -ne 1 ];then
	pkill -9 traffic
        /usr/local/sbin/ats/bin/trafficserver restart
fi
num_zombie=`ps aux | grep -E '\[traffic_server\] <defunct>' | grep -v grep | wc -l`
if [ $num_zombie -eq 1 ];then
        pkill -9 traffic
        /usr/local/sbin/ats/bin/trafficserver restart
fi 
...
if ! ps aux | grep /usr/local/bin/watchdog2 | grep -v grep >/dev/null 2>&1;then
