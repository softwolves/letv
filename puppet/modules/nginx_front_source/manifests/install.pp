class nginx_front_source::install {
  ...
  file { "/home/update/shell/nginx_update.sh":
    ...
    content => template("nginx_front_source/nginx_update.sh"),
  }

  file { "/home/update/puppetmd5file/md5-nginx":
    ensure => file,
    mode => 644,
    content => template("nginx_front_source/md5-nginx"),
  }
  ...
  exec { "update-nginx":
    ...
    refreshonly => true,
    before => Class["nginx_front_source::config"]
  }
}
