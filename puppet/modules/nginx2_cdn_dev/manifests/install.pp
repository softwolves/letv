class nginx2_cdn_dev::install {
  ...
  file { "/home/update/shell/nginx_update.sh":
    ...
    content => template("nginx2_cdn_dev/nginx_update.sh"),
  }

  file { "/home/update/puppetmd5file/md5-nginx":
    ensure => file,
    mode => 644,
    content => template("nginx2_cdn_dev/md5-nginx"),
  }
  ...
  exec { "update-nginx":
    ...
    refreshonly => true,
    before => Class["nginx2_cdn_dev::config"]
  }
}
