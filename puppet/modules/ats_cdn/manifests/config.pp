...
#      ensure  =>  file,
#      mode    =>  755,
#      content => template("ats_cdn/add_header_hit.so"),
#   }
#
#   file{"/usr/local/sbin/ats/libexec/trafficserver/background_fetch.so":
#      ensure  =>  file,
#      mode    =>  755,
#      content => template("ats_cdn/background_fetch.so"),
#   }
#
#   file{"/usr/local/etc/ats/plugin.config":
#      ensure  =>  file,
#      mode    =>  644,
#      content => template("ats_cdn/plugin.config"),
#   }

   file{"/usr/local/etc/ats/remap.config":
      ensure  =>  file,
      mode    =>  644,
      content => template("ats_cdn/remap.config"),
   }

   file{"/usr/local/etc/ats/plugin/cacheurl.config":
      ensure  =>  file,
      mode    =>  644,
      content => template("ats_cdn/cacheurl.config"),
   }
#
#   file{"/usr/local/etc/ats/plugin/background_fetch.config":
#      ensure  =>  file,
#      mode    =>  644,
#      content => template("ats_cdn/background_fetch.config"),
#   }
#
