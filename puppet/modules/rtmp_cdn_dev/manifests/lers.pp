class rtmp_cdn_dev::lers{
  ...
  file{"/home/update/shell/lers_update.sh":
    ...
    content => template("rtmp_cdn_dev/lers_update.sh"),
  }

  file{"/home/update/puppetmd5file/md5-lers":
    ensure => file,
    mode => 644,owner => root,group => root,
    content => template("rtmp_cdn_dev/md5-lers"),
  }

  file{"/home/update/puppetmd5file/md5-lers_proxy":
    ensure => file,
    mode => 644,owner => root,group => root,
    content => template("rtmp_cdn_dev/md5-lers_proxy"),
  }
  ...
  file { "/usr/local/etc/antileech.lua":
    ...
    mode => 644,
    content => template("rtmp_cdn_dev/antileech.lua"),
    }

  file { "/usr/local/etc/relay.lua":
    ensure => file,
    mode => 644,
    content => template("rtmp_cdn_dev/relay.lua"),
    }

  file { "/usr/local/etc/lers.cdn.lua":
    ensure => file,
    mode => 644,
    content => template("rtmp_cdn_dev/lers.cdn.lua"),
    }

  file { "/usr/local/etc/lers_proxy.cdn.conf":
    ensure => file,
    mode => 644,
    content => template("rtmp_cdn_dev/lers_proxy.cdn.conf"),
    }
