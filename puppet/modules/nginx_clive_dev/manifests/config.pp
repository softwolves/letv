...
#  }
  ...
  file{"/usr/local/nginx/conf/win-utf":
    ...
    mode   => 644,
    content => template("nginx_clive_dev/win-utf"),
  }

  file{"/usr/local/nginx/conf/scgi_params":
    ensure => file,
    mode   => 644,
    content => template("nginx_clive_dev/scgi_params"),
  }

  file{"/usr/local/nginx/conf/fastcgi.conf":
    ensure => file,
    mode   => 644,
    content => template("nginx_clive_dev/fastcgi.conf"),
  }

  file{"/usr/local/nginx/conf/nginx.conf":
    ensure => file,
    mode   => 644,
    content => template("nginx_clive_dev/nginx.conf"),
  }

  file{"/usr/local/nginx/conf/mime.types":
    ensure => file,
    mode   => 644,
    content => template("nginx_clive_dev/mime.types"),
  }

  file{"/usr/local/nginx/conf/uwsgi_params":
    ensure => file,
    mode   => 644,
    content => template("nginx_clive_dev/uwsgi_params"),
  }

  file{"/usr/local/nginx/conf/fastcgi_params":
    ensure => file,
    mode   => 644,
    content => template("nginx_clive_dev/fastcgi_params"),
  }

  file{"/usr/local/nginx/conf/koi-win":
    ensure => file,
    mode   => 644,
    content => template("nginx_clive_dev/koi-win"),
  }

  file{"/usr/local/nginx/conf/koi-utf":
    ensure => file,
    mode   => 644,
    content => template("nginx_clive_dev/koi-utf"),
  }
...
####################lua########################
  file{"/usr/local/nginx/conf/lua/hls_creater.lua":
    ...
    mode   => 644,
    content => template("nginx_clive_dev/hls_creater.lua"),
  }

  file{"/usr/local/nginx/conf/lua/init_worker.lua":
    ensure => file,
    mode   => 644,
    content => template("nginx_clive_dev/init_worker.lua"),
  }

  file{"/usr/local/nginx/conf/lua/local_ips.lua":
    ensure => file,
    mode   => 644,
    content => template("nginx_clive_dev/local_ips.lua"),
  }

  file{"/usr/local/nginx/conf/lua/rtmp_connection.lua":
    ensure => file,
    mode   => 644,
    content => template("nginx_clive_dev/rtmp_connection.lua"),
  }

  file{"/usr/local/nginx/conf/lua/limit_test.lua":
    ensure => file,
    mode   => 644,
    content => template("nginx_clive_dev/limit_test.lua"),
  }

#  file{"/usr/local/nginx/conf/lua/proxy.lua":
#    ensure => file,
#    mode   => 644,
#    content => template("nginx_clive_dev/proxy.lua"),
#  }

  file{"/usr/local/nginx/conf/lua/snmpband.lua":
    ensure => file,
    mode   => 644,
    content => template("nginx_clive_dev/snmpband.lua"),
  }

  file{"/usr/local/nginx/conf/lua/live_xml.lua":
    ensure => file,
    mode   => 644,
    content => template("nginx_clive_dev/live_xml.lua"),
  }
...
####################letv#######################
  ...
  file{"/usr/local/nginx/conf/letv/letv_cto.conf":
    ...
    mode   => 644,
    content => template("nginx_clive_dev/letv_cto.conf"),
  }

  file{"/usr/local/nginx/conf/letv/live.conf":
    ensure => file,
    mode   => 644,
    content => template("nginx_clive_dev/live.conf"),
  }

#  file{"/usr/local/nginx/conf/letv/rtmp.conf":
#    ensure => file,
#    mode   => 644,
#    content => template("nginx_clive_dev/rtmp.conf"),
#  }

#  file{"/usr/local/nginx/conf/letv/rtmp_http.conf":
#    ensure => file,
#    mode   => 644,
#    content => template("nginx_clive_dev/rtmp_http.conf"),
#  }

  file{"/usr/local/nginx/conf/lualib/resty/http_headers.lua":
    ensure => file,
    mode   => 644,
    content => template("nginx_clive_dev/http_headers.lua"),
  }

  file{"/usr/local/nginx/conf/lualib/resty/http.lua":
    ensure => file,
    mode   => 644,
    content => template("nginx_clive_dev/http.lua"),
  }

#  file{"/usr/local/nginx/conf/letv/":
#    ensure => file,
#    mode   => 644,
#    content => template("nginx_clive_dev/"),
#  }
