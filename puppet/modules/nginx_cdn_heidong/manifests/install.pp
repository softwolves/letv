class nginx_cdn_heidong::install {
  ...
  file { "/home/update/shell/nginx_update.sh":
    ...
    content => template("nginx_cdn_heidong/nginx_update.sh"),
  }

  file { "/home/update/puppetmd5file/md5-nginx":
    ensure => file,
    mode => 644,
    content => template("nginx_cdn_heidong/md5-nginx"),
  }
  ...
  exec { "update-nginx":
    ...
    refreshonly => true,
    before => Class["nginx_cdn_heidong::config"]
  }
}
