...
CRS_START_INFO_FILE = "/dev/shm/crs.start.info"   #crs启动信息 
...
class CheckCRS():
    ...
    def __init__(self,role):
        ...
        Status    Information
         ...
         -1         未安装,没有CRS相关文件
          ...
          6         CRS/R2H进程CPU利用率达到100%
          7         CRS/R2H进程MEM使用达到5G
        """
    ...
    def checkrestartcount(self, pid, program='crtmpserver', port='1935', infofile=CRS_START_INFO_FILE, threshold=RESTART_THRESHOLD):
        """检测重启阈值"""
        ...
        if not os.path.exists(start_info_file) or len(fileinfo) == 0: #第一次运行或者start_info_file没有内容，需要建立文件并写入当前信息
            ...
            if startinfo['pid'] == fileinfo['pid']:  #pid相同，更新runtime
                ...
                trace_log("runtime is %s,more than threshold: %s."%(fileinfo['runtime'],threshold))
                with open(start_info_file, 'w') as f:
                    f.write(str(startinfo))
                return 0
            elif fileinfo['runtime'] < threshold: #pid不同，runtime小于阈值也要更新
                error_log("runtime is %s,less than threshold: %s."%(fileinfo['runtime'],threshold))
                with open(start_info_file, 'w') as f:
                    f.write(str(startinfo))
                self.code = 4
                return -1

    #def getcputil(self, program='crtmpserver', topnum=3):
    def getcputil(self, program='crtmpserver'):
        """获取program cpu利用率"""
        #cpuper = commands.getoutput("top -b -d 1 -n %s | grep %s | awk 'BEGIN{cpu=0;} {if($9>cpu){cpu=$9;}}  END{print cpu;}'" %(topnum,program))
        cpuper = commands.getoutput("ps aux | grep %s | grep -v grep | awk 'BEGIN{cpu=0;} {if($3>cpu){cpu=$3;}}  END{print cpu;}'" %program)
        trace_log("Get the highest cpu utility %s%%. program %s." %(cpuper,program))
        return "%.2f" %float(cpuper)

    def getmemutil(self, pid, program):
...
if __name__ == "__main__":
    ...
    try:
        ...
        if crs.mode == 1:    # 单进程模式检测 
            ...
            #检测R2H进程在30分钟内是否有重启 
            if role == 2: 
                pid = commands.getstatusoutput("cat %s" % R2H_PID_FILE)[1]
                ret = crs.checkrestartcount(pid, program='rtmp2http', infofile=R2H_START_INFO_FILE)
                if ret != 0:
                    error_log("Restart 2 times in %s seconds." %RESTART_THRESHOLD)
                    code2file(crs.code)
                    sys.exit(1)
            ...
            #检测CRS进程利用率是否达到100%
            cpuper = crs.getcputil(program='crtmpserver')
            if float(cpuper) > 99:
                code2file(6)
                sys.exit(1)
            ...
            #检测R2H进程利用率是否达到100% 
            if role == 2:
                cpuper = crs.getcputil(program='rtmp2http')
                if float(cpuper) > 99:
                    code2file(6)
                    sys.exit(1)

            #监测CRS进程内存使用是否超过5G
            crs_pid = commands.getoutput("cat %s" %CRS_PID_FILE)
            memutil = crs.getmemutil(crs_pid, "crtmpserver")
            if memutil > 5000:
                code2file(7)
                sys.exit(1)
            ...
            #监测R2H进程内存使用是否超过5G
            if role == 2:
                r2h_pid = commands.getoutput("cat %s" %R2H_PID_FILE)
                memutil = crs.getmemutil(r2h_pid, "rtmp2http")
                if memutil > 5000:
                    code2file(7)
                    sys.exit(1)
            ...
            for oneport in crs.ports:
                ...
                ret = crs.checkrestartcount(pid,port=oneport)
                if ret != 0:
                    error_log("Restart 2 times in %s seconds." %RESTART_THRESHOLD)
                    code2file(crs.code)
                    sys.exit(1)
            ...
            for oneport in crs.ports:
                pidfile = "/var/run/crtmpserver.%s.pid" %oneport;
                pid = commands.getoutput("cat %s" %pidfile)
                memutil = crs.getmemutil(pid, "crtmpserver.%s" %oneport)
                if memutil > 5000:
                    code2file(7)
                    sys.exit(1)
