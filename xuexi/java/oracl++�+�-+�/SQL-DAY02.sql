
查询某张表中所有字段的所有记录：
SELECT *FROM emp_lee;
查看指定字段的值
SELECT ename,sal,job,deptno
FROM emp_lee

DQL必须包含 的部分是SELECT子句
与FROM子句。
SELECT用来确定查询的字段，可以
使用的有：表的字段，函数，表达式
FROM子句用来确定查询的表。

SELECT 中使用表达式：
查看每个员工的年薪：
SELECT ename,sal*12
FROM emp_lee

字符串函数：
1：CONCAT(P1，P2)
连接字符串
SELECT CONCAT(ename,sal*12)
FROM emp_lee
SELECT CONCAT(CONCAT(ename,':'),sal)
FROM emp_lee
连接字符串还有一种简便的方式：使用“||”注意与JAVA中区分。
SELECT ename||':'||sal
FROM emp_lee

LENGTH(P)函数
获取字符长度
SELECT ename，LENGTH(ename)
FROM emp_lee
SELECT sal,LENGTH(sal)
FROM emp_lee

UPPER,LOWER,INITCAP
将字符转换为全大写，全小写，首字母大写。
dual：伪表，当查询的内容与任何一张表中的数据无关时，
可以查询伪表。
SELECT UPPER('helloworld'),
       LOWER('HOLLOWORLD'),
       INITCAP('HELLO WROLD')
FROM dual
 
 
 TRIM,LTRIM,RTRIM
 去除字符串两边的指定字符
 SELECT 
 TRIM('e'FROM 'eeeliteee')//只能一个字母
 FROM dual
 SELECT
 LTRIM('eelitee','e')
 FROM dual
 SELECT
 RTRIM('eeeliteee','e')
 FROM dual
SELECT
LTRIM('eesskkkliteeeess','esk')
FROM dual

LPAD,RPAD补位函数
要求显示指定内容指定位数，
若不足则补充若干指定字符以达到要求显示的长度。
SELECT 
ename ,LPAD(sal,5,'$')
FROM emp_lee
SELECT ename ,RPAD(sal,5,' ')
FROM emp_lee
SELECT ename ,RPAD(SAL,3,' ')//截取都是从左忘右
FROM emp_lee
SELECT ename ,LPAD(SAL,3,' ')
FROM EMP_LEE


SUBSTR截取字符串
截取给定字符串，从指定位置开始截取指定个字符。
在数据库中，下标都是从1开始的！！！！
SELECT 
SUBSTR('thinking in java',1,5)
FROM dual
SELECT
SUBSTR('thinking in java',-4,4)//负数表示从倒数位置开始，截取个数不能为负数
FROM dual

INStR(char1,char2[,n[,m]])
查找char2在char1中的位置
n为从指定位置开始查找，可以不写
m为第几次出现，可以不写。
n，m不写默认都是1
SELECT
INSTR('thinking in java','in')
FROM dual
SELECT
INSTR('thinking in java','in',-7,1)
FROM dual

SELECT ename,sal,deptno
FROM emp_lee
WHERE LOWER(ename)='smith'

SELECT ename
FROM emp_lee
WHERE LENGTH(ename)=5

SELECT ename
FROM emp_lee
WHERE SUBSTR(ename,1,1)='A'
SELECT ename
FROM emp_lee
WHERE INSTR(ename,'A')=1

数字函数
1：ROUND(N,[,M])
对n进行四舍五入，保留
到小数点后M位
m可以不写，不写默认为0
M为0则保留到整数位，-1位为10位，依次类推。
SELECT ROUND(45.678，2) FROM dual;
SELECT ROUND(45.678)FROM DUAL;
SELECT ROUND(55.678,-2)FROM DUAL;
SELECT ROUND(45.678,-1)FROM DUAL;

2:TRUNC()函数，参数作用与ROUND
一致，只是不进行四舍五入，而是直接截取
SELECT TRUNC(45.678,2)FROM DUAL;
SELECT TRUNC(45.678，0)FROM DUAL;
SELECT TRUNC(45.678,-1)FROM DUAL;


MOD()函数是求余数
SELECT
  ename,sal,MOD(SAL,1000)
FROM EMP_LEE;

CEIL和FLOOR
向上取整与向下取整
SELECT CEIL(45.678)FROM DUAL
SELECT FLOOR(45.678)FROM DUAL
                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    
日期类型
DATE:7个字节，保存世纪，年月日时分秒。
TIMESTAMP:时间戳，比date多4个字节，可以保存秒以下的精度，
前7个字节与date一致。
关键字：
SYSDATE: 对应一个内置函数，返回一个表示当前系统时间的DATE类型值
SYSTIMESTAMP:同样的，返回的是表示当前系统时间的时间戳类型的值
SELECT SYSDATE FROM dual
DATE默认只显示“DD-MON-RR”
SELECT SYSTIMESTAMP FROM DUAL
时间戳显示的内容较多


TO_DATE函数】
按照给定的日期格式将字符串解析为
DATE类型值。
SELECT
  TO_DATE('1989-09-01  23：22：11','YYYY-MM-DD  HH24:MI:SS')
FROM dual

SELECT
  TO_DATE('1989年09月01日 23:22:11','YYYY"年"MM"月"DD"日" HH24:MI:SS')
FROM DUAL

DATE是可以比较大小的，越晚越大
查看82年以后入职的员工？
SELECT ename,hiredate
FROM emp_lee
WHERE hiredate>TO_DATE('1982-01-01','YYYY-MM-DD')

DATE之间可以做减法，差为相差的天数
产看每个员工至今为止入职多少天了？
SELECT
 ename,TRUNC(SYSDATE-hiredate)
 FROM emp_lee

SELECT
  TRUNC(SYSDATE-TO_DATE('1989-02-26','YYYY-MM-DD'))
FROM dual

DATE可以和一个数字进行加减运算，相当与加减了指定的天数。返回值为对应的日期
7天后是哪天
SELECT
 SYSDATE+7
FROM dual

TO_CAHR
常用于转换日期，可以将日期按照指定的日期格式转换为字符串
SELECT
  TO_CHAR(SYSDATE,'YYYY-MM-DD HH24:MI:SS')
FROM DUAL

SELECT 
  TO_CHAR(
    TO_DATE('50-03-01','RR-MM-DD'),
    'YYYY MM DD'
  )
FROM DUAL

LAST_DAY(DATE)
该函数返回给定日期所在月的最后一天
查看当月月底？
SELECT TO_CHAR(LAST_DAY(SYSDATE),'YY-MM-DD') FROM DUAL
查看每个员工入职所在月的月底？
SELECT ename,LAST_DAY(hiredate)
FROM emp_lee

ADD_MONTHS(date,i)
对给定的日期加上I值对应的月

查看每个员工入职20周年的纪念日？
SELECT 
  ename,ADD_MONTHS(hiredate,12*20)
FROM EMP_LEE

MONTH_BETWEEN(date1，date2)
计算两个日期之间相差的月，计算方式是
使用date1-date2的结果换算的。
SELECT
  ename,TRUNC(MONTHS_BETWEEN(SYSDATE,hiredate))
FROM EMP_LEE 

NEXT_DAY(date,i)
返回给定日期之后一周内的周几
SELECT
 NEXT_DAY(SYSDATE,1)
 FROM DUAL
 
LEAST、GREATEST
求最小值与最大值
这组函数参数不限制数量，两个以上即可。
SELECT
TO_CHAR(LEAST(SYSDATE,TO_DATE('2008-10-10','YYYY-MM-DD')),'YYYY-MM-DD')
FROM DUAL;
 
 查看82年以后入职的员工的入职日期，若是82年以前入职的，
 则显示为：1982-01-01
 SELECT
  ename,TO_CHAR(GREATEST(hiredate,TO_DATE('1982-01-01','YYYY-MM-DD')),'YYYY-MM-DD')
  FROM EMP_LEE
 
 EXTRACT()提取指定日期指定时间分量的值
 SELECT EXTRACT(YEAR FROM SYSDATE)
 FROM DUAL;
 查看81年入职的员工
 SELECT ename，hiredate
 from emp_lee
 WHERE EXTRACT(YEAR FROM hiredate)=1981;
 
 CREATE TABLE student
 (id NUMBER(4),
  name VARCHAR2(20),
  gender CHAR(1)
 );
INSERT INTO student
VALUES(1000,'李莫愁','F');
INSERT INTO student
VALUES(1002，'林平之',NULL)
INSERT INTO student(id,name)
VALUES(1002,'张无忌')
UPDATE STUDENT
SET ID=1001
WHERE NAME='林平之'
判断一个字段的值是否为空，要使用：IS NULL 或 IS NOT NULL
UPDATE STUDENT
SET gender='M'
WHERE gender is NULL

COMMIT
SELECT *FROM STUDENT

NULL的运算
NULL与字符串连接等于什么都没做
NULL与数字运算结果还是NULL
查看每个员工的收入
SELECT ename,sal,comm,sal+comm
FROM EMP_LEE

NVL(p1,p2)
若P1为NULL，函数返回p2
若不为NULL，函数返回p1自身
所以该函数的作用是将NULL值替换为非NULL值。
SELECT ename,sal,comm,sal+NVL(comm,0)
FROM EMP_LEE
查看每个员工的奖金情况：
若奖金不为NULL的则显示为”有奖金“
为NULL的则显示为”没有奖金“
NVL2(p1,p2,p3)
当P1不为NULL，函数返回P2
当p1为NULL，函数返回P3
SELECT ename,comm,NVL2(COMM,'有奖金','没有奖金')
FROM EMP_LEE
NVL2可以完全实现NVL功能，但是反过来则不行。
SELECT ename,NVL2(COMM,COMM+SAL,SAL)
FROM emp_lee




